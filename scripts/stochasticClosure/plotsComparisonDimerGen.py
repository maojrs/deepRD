...
localDataDirectory = os.environ['DATA'] + 'stochasticClosure/dimerGlobal/boxsize' + str(bsize) + '/benchmarkReducedGen'
numModels = 8
...
plotDirectory = os.environ['DATA'] + 'stochasticClosure/dimer/boxsize' + str(bsize) + '/plotsGen/'
try:
...
if plotDistributions:
    ...
    for i in range(3):
        posRef, binEdges = np.histogram(position1_ref[:, i] - position2_ref[:, i], bins=numbins, density=True)
        binsPosRef = 0.5 * (binEdges[1:] + binEdges[:-1])
        velRef, binEdges = np.histogram(velocity1_ref[:, i], bins=numbins, density=True)
        ...
        for j in range(numConditions):
            index = trajIndexes[j]
            pos, binEdges = np.histogram(position1[j][:, i] - position2[j][:, i], bins=numbins, density=True)
            binsPos = 0.5 * (binEdges[1:] + binEdges[:-1])
            vel, binEdges = np.histogram(velocity1[j][:, i], bins=numbins, density=True)
    ...
    # displaying plot
    # plt.tight_layout()
    plt.savefig(plotDirectory + 'distributions_comparison_dimer.pdf')
    plt.clf()
